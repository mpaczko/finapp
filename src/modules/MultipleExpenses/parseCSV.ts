...
function parseAmountFromDescription(description: string) {
  const matches = description.matchAll(/(-?[0-9\s\u00A0,.]+)\s*(?:PLN|zł)\b/gi);
  const amounts = Array.from(matches, (match) => parseAmount(match[1])).filter(
